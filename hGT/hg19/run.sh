...
mkdir blastn-22species blastn-22species/chr22-17038962-17042074 blastn-22species/chr22-18878930-18879701 blastn-22species/chr22-29560816-29561420
for id in `cat 22species.id`
do
    ...
    nohup blastn -task blastn -query chr22-29560816-29561420.fa -db ~/hGT/db/$id.fna -out blastn-22species/chr22-29560816-29561420/$id.blastn -evalue 1e-3 -num_threads 1 -outfmt 7 -word_size 9 &
done


for id in `cat ~/hGT/lastz-seg38M/51species.id`
do
    nohup blastn -task blastn -query chr22-29560816-29561420.fa -db ~/hGT/db/$id.fna -out chr22-29560816-29561420-51species/$id.blastn -evalue 1e-3 -outfmt 7 -word_size 9 &
done
...
awk '{if($4<=8) print $1"-"$2"-"$3}' screenHGT-350seg.out |awk -F '-' '{print $1"\t"$2+$4-1"\t"$2+$5-1}' |sort -k1,1 -k2n,2 |uniq >screenHGT-350seg.bed
~/hGT/src/mergeBed.pl screenHGT-350seg.bed screenHGT-350seg-merge.bed
~/hGT/src/getHGTseq.pl ~/hGT/fa/chr22.fa screenHGT-350seg-merge.bed screenHGT-350seg-merge.fa
RepeatMasker screenHGT-350seg-merge.fa

awk '{if($4<=8) print $1"\t"$2"\t"$3}' screenHGT-chr22.out |sort -k1,1 -k2n,2 |uniq >screenHGT-chr22.bed
~/hGT/src/getHGTseq.pl ~/hGT/fa/chr22.fa screenHGT-chr22.bed screenHGT-chr22.fa
RepeatMasker screenHGT-chr22.fa
...
do
    nohup ~/hGT/src/axt2cov.pl cov/350seg-masked-$id.cov axt/350seg-masked-$id.axt &
    nohup awk '{if($9>=0.6) print $1"\t"$2"\t"$3}' cov/350seg-masked-$id.cov |sort -k1,1 -k2n,2 >iden60/350seg-masked-$id.bed &
    ~/hGT/src/mergeBed.pl iden60/350seg-masked-$id.bed iden60/350seg-masked-$id-merge.bed
    mv iden60/350seg-masked-$id-merge.bed iden60/350seg-masked-$id.bed
done

for id in `cat 12nonmammal.id`
do
    cp iden60/350seg-masked-$id.bed iden60/nonmammal/
done

for id in `cat 39mammal.id`
do
    ~/hGT/src/merge-mammal.pl iden60/350seg-masked-$id.bed iden60/mammal/350seg-masked-$id-merge.txt
done

~/hGT/src/merge-nonmammal.pl 350seg-masked.fa 500 iden60/nonmammal/350seg-masked-merge-cov2-500bp.bed iden60/nonmammal/350seg-masked-*bed
~/hGT/src/screenHGT.pl iden60/nonmammal/350seg-masked-merge-cov2-500bp.bed mm 0.4 iden60/mammal/350seg-masked-*merge.txt
#delete "iden60/mammal/350seg-", "iden60/mammal/chr22-" and "-merge.txt" in the file, then get the real name
~/hGT/src/getName.pl ~/hGT/data/53genome/info.txt screenHGT-350seg-masked.out

awk '{if($4<=8) print $1"-"$2"-"$3}' screenHGT-350seg-masked.out |awk -F '-' '{print $1"\t"$2+$4-1"\t"$2+$5-1}' |sort -k1,1 -k2n,2 |uniq >screenHGT-350seg-masked.bed
~/hGT/src/mergeBed.pl screenHGT-350seg-masked.bed screenHGT-350seg-masked-merge.bed
~/hGT/src/getHGTseq.pl ~/hGT/fa/chr22.fa screenHGT-350seg-masked-merge.bed screenHGT-350seg-masked-merge.fa
RepeatMasker screenHGT-350seg-masked-merge.fa
