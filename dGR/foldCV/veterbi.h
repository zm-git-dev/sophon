...
struct OUT { //scaning result,for sub HMM(TR/VR/RT) or the total DGR
  ...
  //int eff[S]; 
  //whether a VR can be found according to the TR,used only for TR
...
void HMM::print(){
  ...
  for(int i=0;i<size;i++)
    if(state[i] -> length >= len)
      printf("%s(%d-%d:%dbp)\t",name[i],state[i]->pos_start,state[i]->pos_end,state[i]->length);
  printf("\n");
  printf("Start Probabilities\t");
  for(int i=0;i<size;i++)
    if(state[i] -> length >= len)
      printf("%0.3f\t",start[i]);
  printf("\n");
  printf("Ending Probabilities\t");
  for(int i=0;i<size;i++)
    if(state[i] -> length >= len)
      printf("%0.3f\t",end[i]);
  printf("\n");
  printf("Trasnition Probability Matrix\n");
  for(int i=0;i<size;i++){
    if(state[i] -> length >= len){
      for(int j=0;j<size;j++)
	if(state[j] -> length >= len)
	  printf("%0.3f\t",trans[i][j]);
      printf("\n");
    }
  }
  printf("Score Cuttof for a HMM:%0.2f\n",score_cuttof);
}


struct OUT* HMM::scanSeq(char *seq){
  ...
  */
  ...
  if(strlen(seq) > window){
    int length = strlen(seq)-window-1;
    float score[length][size]; //state number:size; plus an end state
    for(int i=0;i<length;i++)
      for(int j=0;j<size+1;j++)
	score[i][j] = 0.0;
    
    for(int i=0;i<length;i++){ //scan the sequence for all the state in all the positions,then set down all the scores
      for(int j=0;j<size;j++){
	if(state[j]->length >= len){
	  float tmp=0.0;
	  for(int k=0;k<state[j]->length;k++)
	    ...
	    case 'a':tmp += state[j]->score[0][k];break;
            ...
            case 'c':tmp += state[j]->score[2][k];break;
            case 'g':tmp += state[j]->score[3][k];break;
	    default:tmp += state[j]->min/state[j]->length; //score for base 'N'
	    }
	  score[i][j] = tmp;
	}
      }
    ...
    int veterbi_start[length][size+1]; //state number:size; ++end state
    float veterbi_score[length][size+1];
    
    for(int i=0;i<length;i++)
      for(int j=0;j<=size;j++){
	veterbi_start[i][j] = 0;
	veterbi_score[i][j] = 0.0;
      }
    
    for(int i=0;i<length;i++){
      if(i==0)
	for(int j=0;j<size;j++)
	  veterbi_score[i][j] = start[j]*score[i][j];
      
      else{
	for(int j=0;j<size;j++){ //calculate transtion state
	  veterbi_start[i][j] = i;
	  veterbi_score[i][j] = start[j]*score[i][j];
	  ...
	  for(;l<i;l++)
	    for(int k=0;k<size;k++){
	      if(i-l >= state[k]->length && trans[k][j] != 0){
		float score_tmp = veterbi_score[l][k] + score[i][j]*trans[k][j];
		if(score_tmp > veterbi_score[i][j]){
		  veterbi_start[i][j] = l;
		  veterbi_score[i][j] = score_tmp;
		}
	      }
	    }
	}
	
	int l = (i>gap)?i-gap:0; //calculate end state
	for(;l<i;l++)
	  for(int k=0;k<size;k++){
	    if(i-l >= state[k]->length && end[k] != 0){
	      float score_tmp = veterbi_score[l][k] + end[k];
	      if(score_tmp > veterbi_score[i][size]){
		veterbi_start[i][size] = l;
		veterbi_score[i][size] = score_tmp;
	      }
	    }
	  }
      }
    ...
    for(int i=0;i<length;i++)
      if(veterbi_score[i][size] > score_cuttof)
	if(veterbi_score[i][size] > max_score){
	  max_score = veterbi_score[i][size];
	  max_start = veterbi_start[i][size];
	  max_end = i;
	}
    if(max_score > score_cuttof){
      result->score[0] = max_score;
      result->start[0] = max_start;
      result->end[0] = max_end;
      result->number = 1;
    }
  }
...
*/
  ...
  int arr_start[S],arr_end[S];
  float arr_score[S];

  int num=0;
  /*scaning for all the HMM models and reserve all the result_tmps*/
  for(int i=0;i<_number;i++){
    struct OUT *result_tmp_sub = hmm[i].scanSeq(seq);
  ...
  q_sort_state(arr_start,arr_end,arr_score,0,num-1);
  ...
  for(int i=0;i<num;i++){
    ...
    else if(!(arr_start[i] > result->end[pos-1] || arr_end[i] < result->start[pos-1])){
      //overlap and merge
...
class SCAN{ //main HMM model used to scan the unknown sequence
 private:
  HMM_class TR; //cluster of TR
  HMM_class VR; //cluster of VR
  HMM_class RT; //cluster of RT
...
void SCAN::init(HMM_class init_TR,HMM_class init_VR,HMM_class init_RT,char *dgr_summary,int init_gap){
  TR=init_TR;VR=init_VR;RT=init_RT;
  gap=init_gap;
...
void SCAN::print(){
  ...
  RT.print();
  printf("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n");
  printf("Start Probability:%0.2f\t%0.2f\t%0.2f\n",start[0],start[1],start[2]);
  printf("End Probability:%0.2f\t%0.2f\t%0.2f\n",end[0],end[1],end[2]);
...
*/
  struct OUT* result_TR = TR.scanSeq(seq);
  if(result_TR->number >0){ //scaning for TR firstly
    struct OUT* result_RT = RT.scanSeq(seq);
    if(result_RT->number >0){ //scaning for RT secondly
      //struct OUT* result_VR = searchVR(seq,&result_TR,1);
      struct OUT* result_VR = VR.scanSeq(seq);
      //if(result_VR->number >0){ //scaning for VR at last

	int number_sub=0,start_sub[S],end_sub[S],type_sub[S];
	float score_sub[S];
	//fetch all the sub matchSequences to some tmp arrayes
	for(int i=0,j=number_sub;i<result_TR->number;i++,j++){
	    start_sub[j] = result_TR->start[i];
	    end_sub[j] = result_TR->end[i];
	    score_sub[j] = result_TR->score[i];
	    type_sub[j] = 1;
	    number_sub += 1;
	}
	for(int i=0,j=number_sub;i<result_VR->number;i++,j++){
	  start_sub[j] = result_VR->start[i];
	  end_sub[j] = result_VR->end[i];
	  score_sub[j] = result_VR->score[i];
	  type_sub[j] = 2;
	  number_sub += 1;
	}
	for(int i=0,j=number_sub;i<result_RT->number;i++,j++){
	  start_sub[j] = result_RT->start[i];
	  end_sub[j] = result_RT->end[i];
	  score_sub[j] = result_RT->score[i];
	  type_sub[j] = 3;
	  number_sub += 1;
	}
	...
	for(int i=0;i<number_sub-1;i++){
	  ...
	  if(pos != i){ //exchange the two matchSeqs
	    ...
	    float tmp4=score_sub[i];
	    start_sub[i]=start_sub[pos];end_sub[i]=end_sub[pos];type_sub[i]=type_sub[pos];score_sub[i]=score_sub[pos];
	    start_sub[pos]=tmp1;end_sub[pos]=tmp2;type_sub[pos]=tmp3;score_sub[pos]=tmp4;
	  }
	}
	...
	for(int i=0;i<number_sub;){
	  if(result->number == 0){
	    ...
	    if(!(start_sub[i] > result->end[pos-1] || end_sub[i] < result->start[pos-1]) && type_sub[i]*result->type[pos-1] == 2){
...
/*Search for VR:
  according to the TR sequence,search for a sequence in the main sequence that satisfy the follow requirements:
  only base A in the TR may mutate in the VR sequence,and other bases(C/T/G) must keep consistent.
  Not the whole main sequence is searched,search space:N bps up and down the corresponding TR(N is the gap length between TR and VR)
*/
/*
struct OUT *searchVR(char *seq,struct OUT **result_TR,int misMatch){
  struct OUT *VR=(struct OUT *)calloc(1,sizeof(struct OUT));
  VR->number=0;
  
  int number=0;
  for(int k=0;k<(*result_TR)->number;k++){ //search for each TR sequence
    (*result_TR)->eff[k] = 0; //default state:for the TR,no VR found
    
    char *TR = substr(seq,(*result_TR)->start[k],(*result_TR)->end[k]-(*result_TR)->start[k]+1);
    
    int len=strlen(TR);
    for(int i=0;i < strlen(seq)-len;i++){
      int error = 0;
      if(i != (*result_TR)->start[k]){
	int index = 0;
	char *sub = substr(seq,i,len);
	for(int j=0;j<len;j++)
	  if(TR[j] != 'A' && sub[j] != TR[j]){ //stop when there is a base(not A) found not assistent
	    error ++;
	    if(error > misMatch){
	      index =1;
	      break;
	...
	if(index == 0){//there may be multi VR found according to the TR,save all the hits
	  (*result_TR)->eff[k] = 1;
	  
	  int start_TR = (*result_TR)->start[k];
	  int end_TR = (*result_TR)->end[k];
	  int left,right;
	  for(left=1;i-left >= 0 && start_TR-left>=0;left++)
	    if(seq[start_TR-left] != 'A' && seq[start_TR-left] != seq[i-left])
	      break;
	  for(right=1;i+right+len-1 < strlen(seq) && end_TR+right < strlen(seq);right++)
            if(seq[end_TR+right] != 'A' && seq[end_TR+right] != seq[i+right+len-1])
              break;
	  left--;
	  right--;
	  
	  (*result_TR)->start[k] -= left;
	  (*result_TR)->end[k] += right;

	  VR->score[number] = 0.0;
	  VR->start[number] = i-left;
	  VR->end[number] = i+len-1+right;
	  printf("TR:%d\tVR:%d\n",(*result_TR)->end[k]-(*result_TR)->start[k],VR->end[number]-VR->start[number]);
	  number++;
	}
	free(sub);
      }
