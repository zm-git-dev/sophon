...
int main(int argc,char* argv[]){
  if(argc < 3){
	...
	if(thread == 1){
	  ...
	  if(out == NULL){
	    char cmd2[100];sprintf(cmd2,"cat %s/out_tmp.txt",tmp);
	    system(cmd2);
	  }
	  else{
	    char cmd2[100];sprintf(cmd2,"cat %s/out_tmp.txt > %s",tmp,out);
	    system(cmd2);
	  }
	}
	else{
	  ...
	  if(out == NULL){
	    char cmd3[100];sprintf(cmd3,"cat %s/out_tmp_*.txt",tmp);
	    system(cmd3);
	  }
	  else{
	    char cmd3[100];sprintf(cmd3,"cat %s/out_tmp_*.txt > %s",tmp,out);
	    system(cmd3);
	  }
	}
	char cmd4[100];sprintf(cmd4,"rm -rf %s",tmp);
	system(cmd4);
      }
    }
  } 
