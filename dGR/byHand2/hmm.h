...
class SCAN{ //main HMM model used to scan the unknown sequence
 ...
 public:
  ...
  void init(HMM_class init_TR,HMM_class init_VR,HMM_class init_RT);
  void print(char *dir);
  struct OUT *scanSeq(char *seq);
};

void SCAN::init(HMM_class init_TR,HMM_class init_VR,HMM_class init_RT){
  ...
  if(scan_sub[0]->number >0){ //scaning for TR firstly
	...
	result->total_start = result->start[0];
	  ...
	  if(product%10 == 0){ //a DGR is found:TR+VR,maybe no VR
	    ...
	    result->number = 0; //reset the result array, start a new search space
	    pos = 0;
	  }
	}
	*/
