...
while(<FILE>){
	...
	if(exists($genome{$id})){
		...
		if($pair != 0){
		    ...
		    for(my $i=0;$i<@sub_seq;$i++){
			if($type[$i] == 3){
			    my $seq_RT = substr($seq,$sub_start[$i],$sub_end[$i]-$sub_start[$i]+1);
			    print OUT "$id\tRT\t$sub_string[$i]\t$sub_start[$i]\t$sub_end[$i]\t$seq_RT\n";
			}
		    }
...
#search for positive string
	...
	if($i+length($TR)-1 < $start_init || $i > $end_init){
		...
		if(length($TR_new) == length($VR_new) && length($TR_new) >= $LEN){
		    #&& ($c>=$b || $d<=$a)){
		#if($mut+$error > 0){
		    my $distance = abs($c-$a);
