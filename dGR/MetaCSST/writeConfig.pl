...
my ($template,$out,$c1_score,$c1_ratio,$c1_len,$c2_score,$c2_ratio,$c2_len,$c3_score,$c3_ratio,$c3_len)= @ARGV;
...
my (@len,@score,@ratio) = ((),(),());
push(@len,$c1_len);push(@len,$c2_len);push(@len,$c3_len);
push(@score,$c1_score);push(@score,$c2_score);push(@score,$c3_score);
push(@ratio,$c1_ratio);push(@ratio,$c2_ratio);push(@ratio,$c3_ratio);
