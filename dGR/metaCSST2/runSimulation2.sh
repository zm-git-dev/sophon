...
do
	...
	for l in ${len[@]}
	do
	    a=$(./scanSub -build $align -score $s -ratio $r -len $l -in $in -tmp $tmp -gap 400 -thread 4 |grep ">" |wc -l)
	    b=$(./scanSub -build $align -score $s -ratio $r -len $l -in random/2kbp.fa -tmp $tmp -gap 400 -thread 4 |grep ">" |wc -l)
