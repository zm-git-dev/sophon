...
struct OUT* HMM::scanSeqSingle(char *seq){
  ...
  */
  ...
  if(strlen(seq) > window){
    for(int i=0;i<strlen(seq)-window-1;){ //scan the sequence for every stat,set down the position as well as matching score
      int index=0;
  ...
  int search_start[S]; //the start index number of the search space
  int search_end[S]; //the end index number of the search space
  for(int i=0;i<S;i++){
    search_start[i] = -1;
    search_end[i] = -1;
  }
